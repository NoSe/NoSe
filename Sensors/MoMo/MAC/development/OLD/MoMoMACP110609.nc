...
/**************************************************/
    ...
    event void SubSend.sendDone(message_t* msg, error_t error) {
        uint8_t num_slot = ( mac_state_ == MT_MM_TX_BR ) ? num_neigh_ : 1;
        if ( error == SUCCESS ) {
            call Leds.led0Off();
            call Leds.led1On();
            mac_state_ = ( mac_state_ == MT_MM_TX_BR ) ? MT_MM_WAIT_ACK_BR : MT_MM_WAIT_ACK_UNI;
            call WaitAckTimer.startOneShot( num_slot * MM_SLOT_LENGTH + call RadioControl.getRxSleepInterval( msg_data_ ) );
            return;
        }
    ...
    event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len) {
        uint8_t my_slot;
        message_t * tmp;
        //MM_mac_header_msg_t* ptr_header = (MM_mac_header_msg_t*) call SubPacket.getPayload( msg, sizeof( MM_mac_header_msg_t ) );
        MM_mac_header_msg_t* ptr_header = (MM_mac_header_msg_t*) payload;
        tmp = msg_recv_data_;
        msg_recv_data_ = msg;
        len = len - sizeof( MM_mac_header_msg_t );
...
/***************************************************/
    ...
    async event void CCAControlAck.requestCca(message_t * ONE msg) {
        //Sending without CCA
        call CCAControlData.setCca( FALSE );
    }
