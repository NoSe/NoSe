...
module MoMoMACP {
    ...
    uses {
        ...
        //MN: used for synchronized transmissions
        /*
        interface TimeSyncAMSend<T32khz, uint32_t> as SubSend;
        interface TimeSyncPacket<T32khz, uint32_t> as CheckDataSync;
        */
        ...
        interface Alarm<TMilli,uint16_t> as SlotTimer;
    }
}
...
/**************************************************/
    ...
    inline void makePktData() {
        ...
        if ( call RadioControl.getRxSleepInterval( msg_data_ ) == 0 )
            sync_time_ = MM_GUARD_TIME;
        else sync_time_ = call RadioControl.getRxSleepInterval( msg_data_ );
        //else sync_time_ = ((int32_t)( call RadioControl.getRxSleepInterval( msg_data_ ) ) << 5);
        
        //MN: to check
        //sync_time_ += call LocalTime.get(); //MN: this should be the time at which the packet has been sent
...
/**************************************************/
	...
	task void sendData() {
	    ...
	    if ( call SubSend.send( call AMPacket.destination( msg_data_ ), msg_data_, msg_data_len_,  sync_time_ ) == SUCCESS ) {
	       call Leds.led0On();
    	   return;
        }
...
/**************************************************/
    ...
    event void SubSend.sendDone(message_t* msg, error_t error) {
        uint8_t num_slot = ( mac_state_ == MT_MM_TX_DATA_BR ) ? num_neigh_ : 1;
        if ( error == SUCCESS ) {
            call Leds.led0Off();
            call Leds.led1On();
            mac_state_ = ( mac_state_ == MT_MM_TX_DATA_BR ) ? MT_MM_WAIT_ACK_BR : MT_MM_WAIT_ACK_UNI;
            //call SlotTimer.start( MM_GUARD_TIME + num_slot * MM_SLOT_LENGTH + ((int32_t)( call RadioControl.getRxSleepInterval( msg ) ) << 5) );
            call SlotTimer.start( num_slot * MM_SLOT_LENGTH + call RadioControl.getRxSleepInterval( msg ) );
            return;
        }
    ...
    event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len) {
        ...
        if ( call CheckDataSync.isValid( msg_recv_data_ ) == TRUE ) {
            ...
            if ( call RadioControl.stopDutyCycle() == SUCCESS ) {
                my_slot = getMySlot( call AMPacket.destination( msg_recv_data_ ) );
                //sync_time_ = call CheckDataSync.eventTime( msg_recv_data_ ) - (call CC2420PacketBody.getMetadata( msg_recv_data_ ))->timestamp + my_slot * MM_SLOT_LENGTH;
                sync_time_ = call CheckDataSync.eventTime( msg_recv_data_ ) - call LocalTime.get() + my_slot * MM_SLOT_LENGTH;
                //MN: to check
                //sync_time_ = (call LocalTime.get() - call CheckDataSync.eventTime( msg_recv_data_ )) + my_slot * MM_SLOT_LENGTH;
                if ( sync_time_ > 0 ) {
                    ...
                    call SlotTimer.start( sync_time_ );
                    call Leds.led0On();
                    return tmp;
                }
            }
